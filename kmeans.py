...
# Choosing `num_centers` random data points as the initial centers
def image_init(dataset, num_centers,points):
    dimension = dataset.size(1)
    selected_indexes = torch.tensor(random.sample(range(0, points), min(points,num_centers)))
    if num_centers>points:
        selected_indexes=torch.cat([selected_indexes,torch.tensor(random.sample(range(0,dataset.shape[0]),num_centers-points))],dim=0)
    centers=dataset.view(-1,points,dimension).mean(dim=0)[selected_indexes[:min(points,selected_indexes.shape[0])]]
    if num_centers>points:
        centers=torch.cat([centers,dataset[selected_indexes[points:]]],dim=0)
    return centers
...
def compute_codes(dataset, centers):
    ...
    chunk_size = int(1e6 / num_centers)
    # chunk_size=3136
...
def cluster(dataset, num_centers,points=3136):
    # centers = random_init(dataset, num_centers)
    centers=image_init(dataset,num_centers,points)
